...
class SunbackMovie:
    ...
    def search_for_images_jsoc(self):
        pass

    def search_for_images_fido(self):
        ...
        self.fido_result = Fido.search(a.Time(self.early, self.now), a.Instrument('aia'), a.Wavelength(int(self.this_name) * u.angstrom),
                                       a.vso.Sample(self.params.cadence()), a.vso.Provider('jsoc2'))
    ...
    def import_fido_information(self):
        ...
        first = self.fido_result.get_response(0)[0]
        last = self.fido_result.get_response(0)[-2]


        time_start = first.time_range().start
        time_end = last.time_range().end

        begin = self.parse_time_string_to_local(''.join(i for i in time_start.iso if i.isdigit()), 2)[0]
        end = self.parse_time_string_to_local(''.join(i for i in time_end.iso if i.isdigit()), 2)[0]

        self.startTime = time_start.iso # #self.parse_time_string_to_local(str(time_start.fits), 2)[0]
        self.endTime =time_end.iso# ''.join(i for i in time_end.iso if i.isdigit())   #self.parse_time_string_to_local(str(time_end.fits), 2)[0]
        # import pdb; pdb.set_trace()

        self.name = first.blocks[0]['wave']['wavemin']
        
        while len(self.name) < 4:
            self.name = '0' + self.name

        if self.params.is_debug():
            self.extra_string = "from {} to {}".format(begin, end)
        else:
            self.extra_string = ''

    def check_downloaded_images(self):
        ...
        for filename in files:
            if filename.endswith(".fits") and "norm" not in filename:
                ...
                self.already_downloaded.append(timeString) #localtime
                # print(str(timeString)[-4:])
                
    def find_missing_images(self):
        """Define what we still need"""
        self.to_get = np.empty(self.fido_result.file_num)
        self.to_get.fill(True)
        for ii, file_to_get in enumerate(self.fido_result.get_response(0)):
            tm = int(file_to_get.time.start[:-2])
            if tm in self.already_downloaded:
    ...
    def get_missing_images(self):
        ...
        if self.new_images:
            ...
            print("    Downloading {} New:".format(self.getNum), flush=True)

            for st in self.get_box:
                out = Fido.fetch(st, path=self.save_path, downloader=Downloader(progress=True, file_progress=False),
                           overwrite=True)
                # print(st)
                # print(out)
