...
```bash
aws_sunback$ sam build --use-container
```
...
```bash
aws_sunback$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
aws_sunback$ sam logs -n HelloWorldFunction --stack-name aws_sunback --tail
```
...
```bash
aws_sunback$ pip install pytest pytest-mock --user
aws_sunback$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name aws_sunback
```
